import React, { useContext, useState } from 'react';
import logo from '../../public/Group.png'
import { IoClose } from 'react-icons/io5';
import { GiHamburgerMenu } from 'react-icons/gi';
import { DashboardResponsiveContext } from '../Context/ResponsiveContext';
const DashNavbar = () => {

    // const [open, setOpen] = useState(false);
    const { viewSideMenu, click } = useContext(DashboardResponsiveContext);
    console.log(click)

    return (
        <div>
            <div className='flex justify-between items-center pr-2 lg:pr-10 border-b-2 py-2 bg-[#F2F2F2]'>
                <div className='flex justify-between items-center w-full'>
                    <div className='flex items-center justify-center'>
                        <div className="flex w-[350px] lg:w-[600px] mx-5 rounded-lg bg-white">
                            <input className=" w-full border-none bg-transparent px-4 py-1  outline-none focus:outline-none text-[#878FA7] " type="search" name="search" placeholder="Search" />
                            <button type="submit" className="m-2 rounded  px-4 py-1  text-white">
                                <svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M9.625 16.625C13.491 16.625 16.625 13.491 16.625 9.625C16.625 5.75901 13.491 2.625 9.625 2.625C5.75901 2.625 2.625 5.75901 2.625 9.625C2.625 13.491 5.75901 16.625 9.625 16.625Z" stroke="#2AA778" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                    <path d="M18.3751 18.375L14.5688 14.5687" stroke="#2AA778" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                            </button>
                        </div>
                    </div>
                    <div onClick={viewSideMenu} className='text-2xl lg:hidden cursor-pointer'>
                        {click ? <IoClose /> : <GiHamburgerMenu />}
                    </div>
                </div>

                <div className='hidden lg:flex justify-between items-center'>
                    {/* notifecation */}

                    <div className='flex m-2 rounded jus justify-center items-center gap-5 px-4 py-1 mr-5 bg-white'>

                        <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M31.7838 21.0912L30.0888 18.2712C29.7138 17.6562 29.3838 16.4712 29.3838 15.7512V12.9462C29.3838 7.50117 24.9588 3.07617 19.5288 3.07617C14.0838 3.09117 9.65884 7.50117 9.65884 12.9462V15.7362C9.65884 16.4562 9.32884 17.6412 8.96884 18.2562L7.27384 21.0762C6.62884 22.1712 6.47884 23.4162 6.88384 24.4962C7.28884 25.5912 8.20384 26.4612 9.40384 26.8512C11.0238 27.3912 12.6588 27.7812 14.3238 28.0662C14.4888 28.0962 14.6538 28.1112 14.8188 28.1412C15.0288 28.1712 15.2538 28.2012 15.4788 28.2312C15.8688 28.2912 16.2588 28.3362 16.6638 28.3662C17.6088 28.4562 18.5688 28.5012 19.5288 28.5012C20.4738 28.5012 21.4188 28.4562 22.3488 28.3662C22.6938 28.3362 23.0388 28.3062 23.3688 28.2612C23.6388 28.2312 23.9088 28.2012 24.1788 28.1562C24.3438 28.1412 24.5088 28.1112 24.6738 28.0812C26.3538 27.8112 28.0188 27.3912 29.6388 26.8512C30.7938 26.4612 31.6788 25.5912 32.0988 24.4812C32.5188 23.3562 32.3988 22.1262 31.7838 21.0912ZM20.6238 15.0012C20.6238 15.6312 20.1138 16.1412 19.4838 16.1412C18.8538 16.1412 18.3438 15.6312 18.3438 15.0012V10.3512C18.3438 9.72117 18.8538 9.21117 19.4838 9.21117C20.1138 9.21117 20.6238 9.72117 20.6238 10.3512V15.0012ZM23.7445 30.015C23.1145 31.755 21.4495 33 19.4995 33C18.3145 33 17.1445 32.52 16.3195 31.665C15.8395 31.215 15.4795 30.615 15.2695 30C15.4645 30.03 15.6595 30.045 15.8695 30.075C16.2145 30.12 16.5745 30.165 16.9345 30.195C17.7895 30.27 18.6595 30.315 19.5295 30.315C20.3845 30.315 21.2395 30.27 22.0795 30.195C22.3945 30.165 22.7095 30.15 23.0095 30.105C23.2495 30.075 23.4895 30.045 23.7445 30.015Z" fill="#2AA778" />
                        </svg>

                        <svg width="27" height="25" viewBox="0 0 27 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M26.9484 5.35986L17.4782 14.8301C16.4222 15.8833 14.9916 16.4748 13.5002 16.4748C12.0087 16.4748 10.5781 15.8833 9.52219 14.8301L0.0519939 5.35986C0.036244 5.53761 0.000244141 5.69849 0.000244141 5.87511V19.375C0.00203047 20.8663 0.595233 22.296 1.64973 23.3505C2.70423 24.405 4.13393 24.9982 5.62522 25H21.3751C22.8664 24.9982 24.2961 24.405 25.3506 23.3505C26.4051 22.296 26.9983 20.8663 27.0001 19.375V5.87511C27.0001 5.69849 26.9641 5.53761 26.9484 5.35986Z" fill="#2AA778" />
                            <path d="M15.8876 13.2393L26.1633 2.96248C25.6655 2.1371 24.9634 1.45391 24.1248 0.978815C23.2861 0.503722 22.3392 0.252755 21.3753 0.250122H5.62538C4.66151 0.252755 3.71458 0.503722 2.87592 0.978815C2.03727 1.45391 1.33519 2.1371 0.837402 2.96248L11.1131 13.2393C11.7471 13.8708 12.6055 14.2253 13.5003 14.2253C14.3952 14.2253 15.2536 13.8708 15.8876 13.2393Z" fill="#2AA778" />
                        </svg>


                    </div>

                    {/* border */}

                    <div className='bor border-l-4 '>

                    </div>


                    {/* Login/Logout */}


                    <div className='flex justify-center items-center gap-2 pl-4'>

                        <h1 className=' font-bold'>Hello,BDRCS</h1>

                        <div className='flex justify-center items-center gap-1 px-2'>
                            <img src={logo} alt="logo" className='w-8' />
                            <svg width="12" height="9" viewBox="0 0 12 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M1 1.88916L5.8891 6.77826L10.7782 1.88916" stroke="#878FA7" stroke-opacity="0.43" stroke-width="2" stroke-linecap="round" />
                            </svg>

                        </div>


                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M7.66667 10.6667C7.40145 10.6667 7.1471 10.772 6.95956 10.9596C6.77202 11.1471 6.66667 11.4015 6.66667 11.6667V12.2C6.66667 12.6774 6.47702 13.1352 6.13946 13.4728C5.80189 13.8104 5.34406 14 4.86667 14H3.8C3.32261 14 2.86477 13.8104 2.52721 13.4728C2.18964 13.1352 2 12.6774 2 12.2V3.8C2 3.32261 2.18964 2.86477 2.52721 2.52721C2.86477 2.18964 3.32261 2 3.8 2H4.86667C5.34406 2 5.80189 2.18964 6.13946 2.52721C6.47702 2.86477 6.66667 3.32261 6.66667 3.8V4.33333C6.66667 4.59855 6.77202 4.8529 6.95956 5.04044C7.1471 5.22798 7.40145 5.33333 7.66667 5.33333C7.93188 5.33333 8.18624 5.22798 8.37377 5.04044C8.56131 4.8529 8.66667 4.59855 8.66667 4.33333V3.8C8.66561 2.7925 8.26491 1.82657 7.5525 1.11416C6.84009 0.401755 5.87416 0.00105885 4.86667 0L3.8 0C2.7925 0.00105885 1.82657 0.401755 1.11416 1.11416C0.401755 1.82657 0.00105885 2.7925 0 3.8L0 12.2C0.00105885 13.2075 0.401755 14.1734 1.11416 14.8858C1.82657 15.5982 2.7925 15.9989 3.8 16H4.86667C5.87416 15.9989 6.84009 15.5982 7.5525 14.8858C8.26491 14.1734 8.66561 13.2075 8.66667 12.2V11.6667C8.66667 11.4015 8.56131 11.1471 8.37377 10.9596C8.18624 10.772 7.93188 10.6667 7.66667 10.6667Z" fill="#444444" />
                            <path d="M15.0405 6.34996L11.9832 3.29263C11.8903 3.19978 11.78 3.12614 11.6587 3.07591C11.5373 3.02568 11.4073 2.99985 11.2759 2.99988C11.1446 2.99991 11.0146 3.02581 10.8932 3.07609C10.7719 3.12638 10.6617 3.20007 10.5688 3.29296C10.3813 3.48056 10.276 3.73496 10.2761 4.0002C10.2761 4.13153 10.302 4.26157 10.3523 4.3829C10.4026 4.50422 10.4763 4.61445 10.5692 4.7073L12.8432 6.9813L4.6665 6.99996C4.40129 6.99996 4.14693 7.10532 3.9594 7.29286C3.77186 7.48039 3.6665 7.73475 3.6665 7.99996C3.6665 8.26518 3.77186 8.51953 3.9594 8.70707C4.14693 8.89461 4.40129 8.99996 4.6665 8.99996L12.8785 8.9813L10.5672 11.2926C10.3796 11.4801 10.2741 11.7345 10.2741 11.9997C10.274 12.265 10.3793 12.5194 10.5668 12.707C10.7543 12.8946 11.0087 13 11.2739 13C11.5392 13.0001 11.7936 12.8948 11.9812 12.7073L15.0385 9.64996C15.4756 9.2122 15.7213 8.61899 15.7216 8.00038C15.722 7.38177 15.4771 6.78825 15.0405 6.34996Z" fill="#444444" />
                        </svg>

                        <h1 className='text-[#444]  font-semibold'>Log out</h1>

                    </div>
                </div>

            </div>
        </div>
    );
};

export default DashNavbar;